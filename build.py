...
def clean_lines(lines):
    ...
    for line in lines:
        ...
        if inside_ascii_art:
            ...
            stripped_line = re.sub(r'#.*', '', line).strip()  # Remove anything after '#' (comments)
...
def combine_powershell_scripts(source_file, modules_folder, output_file):
    ...
    module_pattern = re.compile(r'^\.\s+(.*\\Modules\\.*\.ps1)$')  # Match module import lines that start with '. '
        ...
        if inside_show_dragon:
            ...
            if module_match:
                ...
                if module_full_path.exists():
                    ...
                    combined_script.extend(module_lines)  # Keep the original lines without cleaning them
            else:
